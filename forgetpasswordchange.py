...
def apply():
    ...
    if passs==confirmpass:
        ...
        conn.commit()
        conn.close()
        messagebox.showinfo("Success","Password successfully go back and login in again")
        root.destroy()
        os.system("python User_Loginn.py")
    elif passs!=confirmpass:
        messagebox.showwarning("Error","Password Doesn't match")
    else:
        messagebox.showwarning("Username Error","Invaild Usernane")
...
loginFrame = ctk.CTkFrame(root, corner_radius=50, width=600, height=500, bg_color='#7E9779', fg_color='#262724')
...
Apply_button = ctk.CTkButton(loginFrame, fg_color="#7B9976", text_color="#262724", text='Apply',
