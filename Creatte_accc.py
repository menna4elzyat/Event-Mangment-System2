...
def summit():
    if (Enter_first_name.get()=="" or Enter_last_name.get()=="" or Enter_username.get()=="" or Enter_password.get()==""or Enter_mother_name.get()==""):
        CTkMessagebox(title="warning",message="Enter all fileds",button_color="#7B9976",button_hover_color="#7B9976")
        return
    else:
        conn = sqlite3.connect("database.db")
        createaccount = conn.cursor()
        createaccount.execute("INSERT into user(first_name,last_name,username,password,mother_name) VALUES(?,?,?,?,?)",
                              (Enter_first_name.get(),Enter_last_name.get(),Enter_username.get(),Enter_password.get(),Enter_mother_name.get()))
        conn.commit()
        conn.close()
        messagebox.showinfo("success", "Succcessfully created, go and log in again")
        root.destroy()
        os.system("python User_Loginn.py")
