...
def submit():
    ...
    createconncetion.execute(
        "INSERT INTO event(event_name,event_id,event_date,event_time,event_duration) VALUES(?,?,?,?,?)", (entryname.get(),entryid.get(),entrydate.get(),entryTime.get(),entryduration.get()))
    conn.commit()
    conn.close()
    messagebox.showinfo("Success!!", "Event have created successfully!!")
    root.destroy()
    os.system("python adminpage.py")
...
loginFrame = ctk.CTkFrame(root, corner_radius=60, width=700, height=600, bg_color='#7E9779', fg_color='#262724')
...
entrydate = ctk.CTkEntry(loginFrame, width=300, height=30, font=('Times New Roman', 20, 'bold'),
                         placeholder_text='Enter Date as Friday in cairo')
entrydate.place(x=350, y=275)
...
but_submit = ctk.CTkButton(loginFrame, text='Submit', text_color='#262724', fg_color='#7B9976', hover_color='#7B9976',
...
but_delete = ctk.CTkButton(loginFrame, text='Delete', text_color='#262724', fg_color='#7B9976', hover_color='#7B9976',
...
but_back = ctk.CTkButton(loginFrame, text='', fg_color='#262724', hover_color='#262724', image=back_image, width=40,
                         command=back
                         )
